    ...
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                Downsample1d(dim_out) if not is_last and horizon > 2 else nn.Identity()
            ]))

            if not is_last and horizon > 2:
                horizon = horizon // 2
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            self.ups.append(nn.ModuleList([
                ...
                Upsample1d(dim_in) if not isinstance(self.downs[len(in_out) - ind - 2][-1], nn.Identity) else nn.Identity()
            ]))

            if not isinstance(self.downs[len(in_out) - ind - 2][-1], nn.Identity):
                horizon = horizon * 2
...
class MLPnet(nn.Module):
    ...
    def forward(self, x, cond, time, returns=None, use_dropout=True, force_dropout=False):
        ...
        if self.returns_condition:
            assert returns is not None
            returns_embed = self.returns_mlp(returns).unsqueeze(1)
            if use_dropout:
                mask = self.mask_dist.sample(sample_shape=(returns_embed.size(0), 1, 1)).to(returns_embed.device)
                returns_embed = mask*returns_embed
            if force_dropout:
                returns_embed = 0*returns_embed
            t = torch.cat([t, returns_embed], dim=-1)
